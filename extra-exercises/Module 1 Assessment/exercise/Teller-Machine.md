...
6. Override the `ToString()`/`toString()` method and have it return `"ATM - {manufacturer} - {balance}"` where `{manufacturer}` and `{balance}` are placeholders for the actual values. The values from the object must be shown in the string where `{variable-name}` is indicated.
