...
-- WHEN DEALING WITH NULL RECORDS
...
SELECT city_name
FROM city
WHERE city_name IN ('Columbus','Cleveland','Akron');
...
FROM park
WHERE area BETWEEN 108 AND 200;

-- THE QUERY ABOVE IS THE SAME RESULTSET AS BELOW
-- SELECT park_name, area
-- FROM park
-- WHERE area >= 108 AND area <= 200;
...
FROM park
WHERE date_established < '1/1/1900'; -- month/day/year

SELECT park_name, date_established
FROM park
WHERE date_established < '1900-01-01'; -- year - month - day
