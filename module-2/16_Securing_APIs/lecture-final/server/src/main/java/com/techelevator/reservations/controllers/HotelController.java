...
Three annotations to highlight:
@PreAuthorize("isAuthenticated()") - Requires that a user must have a valid token. Can put on a method or a class
@PreAuthorize("permitAll") - permits all requests to call a method
@PreAuthorize("hasRole('ADMIN')") - allows only users with a specific role to call the method

Principal - include this in your method parameters and it will automatically be provided. You can call
            getName() on the principal and get the username of the who is logged in via the token
...
public class HotelController {
    ...
    public Reservation addReservation(@Valid @RequestBody Reservation reservation, Principal principal) {
        reservation.setCreatedBy( principal.getName());
